...
const Outlets = () => {
    ...
    if (error) return (
        <div className="container py-5 text-center">
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="alert alert-danger shadow-premium rounded-4 p-5">
                <i className="bi bi-exclamation-octagon fs-1 d-block mb-3 text-danger"></i>
                <h3 className="fw-bold">Connection Issue</h3>
                <p className="text-muted mb-4">{error}</p>
                <button className="btn btn-orange rounded-3 px-5 py-2" onClick={() => window.location.reload()}>
                    <i className="bi bi-arrow-clockwise me-2"></i> Try Reconnect
                </button>
            </motion.div>
        </div>
    );
